...
class DisplayClass(object):
    ...
    def __init__(self, title, column_list):
        ...
        self._columns = []
        self._display_column_count = 0
        self._confirm = None # confirm column
        self._confirm_flag = False # flag to add confirm column
        self._reply = None # reply column
        self._reply_flag = False # flag to add reply column
        self.load_columns()
    ...
    def load_columns(self):        
        ...
        # set last of displays
        self._display_column_count = len(self._columns)                
                
        if self._confirm_flag:
            #print("Add Confirm")
            # build Yes/No confirm
            module = __import__("Column_SmallList")
            class_ = getattr(module, "SmallListColumn")
            #"column_first_line":32, "text_start_row": 80, "start_column":60, "end_column":240, "center":True
            self._columns.append(class_(self._draw, None, 32, 80, 60, 180, True, ["Yes", "No"]))            
            self._confirm = len(self._columns) - 1
        if self._reply_flag:
            # Text for errors or confirmation
            #print("Add Reply")
            module = __import__("Column_Text")
            class_ = getattr(module, "TextColumn")
            #"column_first_line":32, "text_start_row": 80, "start_column":60, "end_column":240, "center":True                
            self._columns.append(class_(self._draw, None, 32, 80, 60, 180, True, ["Pending"]))
            self._reply = len(self._columns) - 1
        #print("Len Columns:", len(self._columns), self._confirm_flag, self._reply_flag)        

    def confirm(self, values):
        # action to perform if "Yes" confirm is selected
        # This must be overridden to do anything
        print("Class Confirm - likely shouldn't be here", values)
    ...
    def display_columns(self, action):
        ...
        elif self._active_column == self._reply:
            # in confirm column
            self._columns[self._reply].get_active_image(action)
        else:
    ...
    def right(self, action):
        ...
        time = None
        action = None
        print(len(self._columns), self._display_column_count, self._active_column)
        #print("Conf Reply flags:", self._confirm_flag, self._reply_flag)
        
        if self._active_column in [self._confirm, self._reply]:
            # in confirm screen, so can move
            #print("Right - in confirm/reply", self._active_column, self._confirm)
            action = RIGHT
        ...
        if self._active_column == self._confirm:
            ...
            if self._columns[self._confirm]._selection == "No":
                ...
                if self._reply_flag:
                    action = None
                    #print("Status", action)
                    # error in confirm handling
                    # move back to display columns
                    self._active_column = 0
                    # push new text down to reply column
                    #print("Text", self._text)
                    self._columns[self._reply]._text = self._reply_text
                    self._active_column = self._reply
                    self.init(action)
                #print("Center exit", action)
                return self._image, None, action
        elif self._active_column == self._reply:
            # # exit from reply by going back
            ...
            if self._confirm:
                ...
                # have reply but no confirm - process action
                values = []
                for x in range(0, self._display_column_count):
                    values.append(self._columns[x]._selection)
                # call confirm process
                status = self.confirm(values)
                action = None # exit on success or no return
                
                # proceed to display of reply
                self._active_column = self._reply
                self.init(action)
                # initialize confirm
                self._columns[self._reply]._text = self._reply_text
                self._columns[self._reply].init(action)
                image, time, action = self._columns[self._active_column].receive(INIT)
                # no confirm, but have reply
                # assume text set by validate
            else:
                # pass on to column
                #print("Center - else")
